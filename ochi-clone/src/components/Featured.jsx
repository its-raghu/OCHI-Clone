...
function Featured() {
  ...
  return (
    <div className="w-full rounded-t-3xl">
      ...
      <div className="w-full h-screen flex gap-4 p-16">
        <motion.div onHoverStart={() => handleHover(0)} onHoverEnd={() => handleHoverEnd(0)} className="w-1/2 relative">
          <div>
            <h1 className="absolute flex overflow-hidden top-1/2 left-full -translate-x-1/2 -translate-y-1/2 z-10 text-[9vw] font-Grotesk uppercase text-[#CDEA68] whitespace-nowrap">
              {"fyde".split("").map((items, index) => (
                <motion.span initial={{ y: "100%" }} animate={cards[0]} transition={{ ease: [0.33, 1, 0.68, 1], delay: index * 0.07 }}>
                  {items}
                </motion.span>
              ))}
            </h1>
          ...
          {["audit", "copywriting", "sales desk", "slides design"].map((items, index) => {
            return <button className="border-[1px] border-gray-500 px-3 py-1 mr-3 mt-4 uppercase font-Neue text-xl rounded-full">{items}</button>;
          })}
        </motion.div>
          ...
          <div>
            <h1 className="absolute flex overflow-hidden top-1/2 right-full translate-x-1/2 -translate-y-1/2 z-10 text-[9vw] font-Grotesk uppercase text-[#CDEA68] whitespace-nowrap">
              {"vise".split("").map((items, index) => (
                <motion.span initial={{ y: "100%" }} animate={cards[1]} transition={{ ease: [0.33, 1, 0.68, 1], delay: index * 0.07 }}>
                  {items}
                </motion.span>
              ))}
            </h1>
      ...
      <div className="w-full h-screen flex gap-4 p-16 mb-32">
        <motion.div onHoverStart={() => handleHover(0)} onHoverEnd={() => handleHoverEnd(0)} className="w-1/2 relative">
          <div>
            <h1 className="absolute flex overflow-hidden top-1/2 left-full -translate-x-1/2 -translate-y-1/2 z-10 text-[9vw] font-Grotesk uppercase text-[#CDEA68] whitespace-nowrap">
          ...
          <div className="w-full h-[38vw] overflow-hidden rounded-2xl hover:scale-95 ease-in-out duration-700 relative">
            <div className='w-full h-[38vw] bg-[url("https://ochi.design/wp-content/uploads/2023/08/Frame-3875-663x551.jpg")] bg-cover hover:scale-110 ease-in-out duration-700'></div>
          </div>
